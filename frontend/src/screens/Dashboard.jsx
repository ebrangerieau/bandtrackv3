...
export default function Dashboard({ palette, user, navigate }) {
  ...
  useEffect(() => {
    Promise.all([api.getGroup(), api.getSongs(), api.getGigs(), api.getActivity()])
      .then(([g, s, gi, a]) => {
        setGroup(g)
        setSongs(s)
        setGigs(gi)
  ...
  if (loading) {
    return (
      <div style={{ position: 'absolute', inset: 0, background: palette.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ color: palette.muted, fontSize: 14 }}>Chargement…</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: palette.bg, color: palette.text, fontFamily: '"Plus Jakarta Sans", system-ui, sans-serif', fontSize: 14 }}>
      ...
      <div style={{
      ...
      }} className="bt-no-scroll">
        {/* Header */}
        <div style={{ padding: '14px 20px 10px', display: 'flex', alignItems: 'center', gap: 12, position: 'relative' }}>
          <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, color: palette.muted, fontWeight: 500, letterSpacing: 0.3, textTransform: 'uppercase' }}>
              {group?.name || 'Mon groupe'}
            </div>
            <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: -0.3 }}>
              Bonjour {user.name} <span style={{ display: 'inline-block', transform: 'rotate(15deg)' }}>👋</span>
            </div>
          </div>
          <button style={{
            width: 40, height: 40, borderRadius: 12, border: `1px solid ${palette.border}`,
            background: alpha(palette.surface2, 0.8), color: palette.text, cursor: 'pointer',
        ...
        <div style={{ padding: '6px 20px 0', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {nextGig ? (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative' }}>
                ...
                }}>{nextGig.weekday} {nextGig.display}</div>
              </div>
              <div style={{ marginTop: 10, fontSize: 25, fontWeight: 800, letterSpacing: -0.6, lineHeight: 1.1, position: 'relative' }}>
                {nextGig.name}
              </div>
              <div style={{ marginTop: 4, fontSize: 13, color: palette.muted, fontWeight: 500 }}>📍 {nextGig.place}</div>
              <div style={{ display: 'flex', gap: 6, marginTop: 14 }}>
                <Chip color={palette.accent3}>♪ {nextGig.setlist_count} morceaux</Chip>
                <Chip color="#fbbf24">⏱ {nextGig.duration} min</Chip>
                <Chip color={palette.accent2}>J−{nextGig.dday}</Chip>
              </div>
            </div>
          ) : (
            <div style={{
              ...
              textAlign: 'center', color: palette.muted, fontSize: 13,
            }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>🎸</div>
              Aucun concert à venir
              <button onClick={() => navigate('prestations')} style={{
                display: 'block', margin: '10px auto 0', padding: '8px 16px',
                borderRadius: 10, border: `1px solid ${alpha(palette.accent, 0.4)}`,
                background: alpha(palette.accent, 0.12), color: palette.accent,
                fontSize: 12, fontWeight: 700, cursor: 'pointer',
              }}>+ Créer une prestation</button>
            </div>
          )}
          ...
          <div className="bt-stagger" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            {[
              { n: learning.length, label: 'En cours', color: STATUS.learning.color, tab: 'repertoire' },
              { n: ready.length,    label: 'Prêts',    color: STATUS.ready.color,    tab: 'repertoire' },
              { n: ideas.length,    label: 'Idées',    color: STATUS.suggestion.color, tab: 'repertoire' },
            ].map(s => (
              <div key={s.label} onClick={() => navigate(s.tab)} style={{
                ...
                <span style={{
                  ...
                  background: s.color, opacity: 0.8, boxShadow: `0 0 8px ${alpha(s.color, 0.7)}`,
                }} />
          ...
          {/* Learning songs scroll */}
          {learning.length > 0 && (
            <div>
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 9 }}>
                <span style={{ fontSize: 14, fontWeight: 700, letterSpacing: -0.2 }}>Morceaux en cours</span>
                <span onClick={() => navigate('repertoire')} style={{ fontSize: 12, color: palette.accent, fontWeight: 600, cursor: 'pointer' }}>Voir tout →</span>
              </div>
              <div className="bt-stagger bt-no-scroll" style={{ display: 'flex', gap: 9, overflowX: 'auto', margin: '0 -20px', padding: '0 20px 4px' }}>
                {learning.slice(0, 5).map(s => {
                  const avg = s.avg_level || 0
                  return (
                    <div key={s.id} onClick={() => navigate('morceau', { songId: s.id })} style={{
                      flex: '0 0 auto', width: 140, height: 92,
                      background: palette.surface, borderRadius: 14,
                      border: `1px solid ${palette.border}`,
                      padding: '10px 12px', position: 'relative', overflow: 'hidden', cursor: 'pointer',
                    }}>
                      ...
                      <div style={{ position: 'absolute', bottom: 8, left: 12, right: 12, display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                        <span style={{ fontSize: 18, fontWeight: 800, letterSpacing: -0.5, fontVariantNumeric: 'tabular-nums' }}>{avg.toFixed(1)}</span>
            ...
            <div style={{ paddingBottom: 8 }}>
              ...
              <div style={{ background: palette.surface, borderRadius: 16, border: `1px solid ${palette.border}`, overflow: 'hidden' }}>
                {activity.slice(0, 5).map((a, i) => {
                  const dotColor = a.status_type === 'accent'  ? palette.accent
                                 : a.status_type === 'accent3' ? palette.accent3
                                 : a.status_type === 'suggestion' ? STATUS.suggestion.color
                                 : a.status_type === 'learning'   ? STATUS.learning.color
                                 : a.status_type === 'ready'      ? STATUS.ready.color
                                 : a.status_type === 'archived'   ? STATUS.archived.color
                                 : palette.accent
                  return (
                    <div key={a.id} style={{
                      ...
                      <div style={{ flex: 1, fontSize: 12.5, lineHeight: 1.35, minWidth: 0 }}>
                        ...
                        <span style={{ color: palette.muted }}>{a.description}</span>
                      </div>
                      <span style={{ fontSize: 10.5, color: palette.dim, fontWeight: 500, flexShrink: 0 }}>{a.time_ago}</span>
                    </div>
                  )
                })}
              </div>
      ...
      <TabBar active="dashboard" palette={palette} badge={activity.length > 0 ? activity.length : null} onNavigate={navigate} />
